...
use BlackCat\Database\Packages\RbacUserRoles\Repository\RbacUserRoleRepositoryInterface;
...
class RbacUserRoleRepository implements RbacUserRoleRepositoryInterface, RepoContract, KeysetRepoContract
{
use OrderByTools, PkTools, RepositoryHelpers;

    /** @var mixed literal token for upsert keys (array or empty). */
    private mixed $tokenUpsertKeys = [ 'user_id', 'role_id', 'tenant_id', 'scope' ];
    ...
    public function upsertMany(array $rows): int {
        ...
        if ($helperKeys && class_exists(\BlackCat\Database\Support\BulkUpsertHelper::class)) {
          $bulk = new \BlackCat\Database\Support\BulkUpsertHelper($this->db, \BlackCat\Database\Packages\RbacUserRoles\Definitions::class);
          $bulk->upsertMany($rows, $helperKeys, [ 'status', 'granted_by', 'granted_at', 'expires_at' ]);
          return count($rows);
        }
    ...
    public function getByUserIdAndRoleIdAndTenantIdAndScope(int $userId, int $roleId, int $tenantId, string $scope, bool $asDto = false): array|\BlackCat\Database\Packages\RbacUserRoles\Dto\RbacUserRoleDto|null {
        return $this->getByUnique([ 'user_id' => $userId, 'role_id' => $roleId, 'tenant_id' => $tenantId, 'scope' => $scope ], $asDto);
    }
    ...
    public function getIdByUserIdAndRoleIdAndTenantIdAndScope(int $userId, int $roleId, int $tenantId, string $scope) {
        $row = $this->getByUserIdAndRoleIdAndTenantIdAndScope($userId, $roleId, $tenantId, $scope, false);
