...
namespace BlackCat\Database\Packages\RbacUserRoles\Repository;

use BlackCat\Core\Database as Database;              // e.g. BlackCat\Core\Database
use BlackCat\Database\Packages\RbacUserRoles\Definitions;
use BlackCat\Database\Packages\RbacUserRoles\Criteria;
use BlackCat\Database\Packages\RbacUserRoles\Dto\RbacUserRoleDto as Dto;
use BlackCat\Database\Packages\RbacUserRoles\Mapper\RbacUserRoleDtoMapper as RowMapper;
...
final class RbacUserRoleRepository implements RepoContract, KeysetRepoContract
{
     ...
     */
    protected function def(): string { return \BlackCat\Database\Packages\RbacUserRoles\Definitions::class; }
    ...
    public function upsertMany(array $rows): int {
        ...
        if (class_exists(\BlackCat\Database\BulkUpsertRepository::class) && [ 'user_id', 'role_id', 'tenant_id', 'scope' ] !== []) {
            $bulk = new \BlackCat\Database\BulkUpsertRepository($this->db, \BlackCat\Database\Packages\RbacUserRoles\Definitions::table());
            return $bulk->upsertMany($rows, [ 'user_id', 'role_id', 'tenant_id', 'scope' ], [ 'status', 'granted_by', 'granted_at', 'expires_at' ]);
        }
    ...
    */
    ...
    /** @return array<string,mixed>|\BlackCat\Database\Packages\RbacUserRoles\Dto\RbacUserRoleDto|null */
    public function getByUserIdAndRoleIdAndTenantIdAndScope(int $userId, int $roleId, int $tenantId, string $scope, bool $asDto = false): array|\BlackCat\Database\Packages\RbacUserRoles\Dto\RbacUserRoleDto|null {
        $row = $this->getByUnique([ 'user_id' => $userId, 'role_id' => $roleId, 'tenant_id' => $tenantId, 'scope' => $scope ]);
        if (!$asDto || !$row) return $row;
        return \BlackCat\Database\Packages\RbacUserRoles\Mapper\RbacUserRoleDtoMapper::fromRow($row);
    }
    public function existsByUserIdAndRoleIdAndTenantIdAndScope(int $userId, int $roleId, int $tenantId, string $scope): bool {
        $where = 't.' . Ident::q($this->db, 'user_id') . ' = :uniq_user_id' . ' AND ' . 't.' . Ident::q($this->db, 'role_id') . ' = :uniq_role_id' . ' AND ' . 't.' . Ident::q($this->db, 'tenant_id') . ' = :uniq_tenant_id' . ' AND ' . 't.' . Ident::q($this->db, 'scope') . ' = :uniq_scope';
        return $this->exists($where, [ 'uniq_user_id' => $userId, 'uniq_role_id' => $roleId, 'uniq_tenant_id' => $tenantId, 'uniq_scope' => $scope ]);
    }
    /** @return int|string|null */
    public function getIdByUserIdAndRoleIdAndTenantIdAndScope(int $userId, int $roleId, int $tenantId, string $scope) {
        $row = $this->getByUserIdAndRoleIdAndTenantIdAndScope($userId, $roleId, $tenantId, $scope);
        return $row ? ($row['id'] ?? null) : null;
    }
    /** @return array<string,mixed>|\BlackCat\Database\Packages\RbacUserRoles\Dto\RbacUserRoleDto|null */
    public function getById(int $id, bool $asDto = false): array|\BlackCat\Database\Packages\RbacUserRoles\Dto\RbacUserRoleDto|null {
    ...
    /** @return array<string,mixed>|\BlackCat\Database\Packages\RbacUserRoles\Dto\RbacUserRoleDto|null */
    public function getByIdForTenant(int|string $id, int|string $tenantId, bool $asDto = false): array|\BlackCat\Database\Packages\RbacUserRoles\Dto\RbacUserRoleDto|null {
