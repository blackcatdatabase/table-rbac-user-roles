...
namespace BlackCat\Database\Packages\RbacUserRoles\Service;

use BlackCat\Core\Database;
use BlackCat\Database\Packages\RbacUserRoles\Dto\RbacUserRoleDto;
use BlackCat\Database\Packages\RbacUserRoles\Mapper\RbacUserRoleDtoMapper;
 ...
 */
final class RbacUserRolesAggregateService
{
    use ServiceHelpers;
    ...
    public function __construct(
        private Database $db, private RbacUserRoleRepository $rbacUserRoleRepo
    ) {
        // Repository autowiring (generated)
        $this->repositories = [
  'rbac_user_roles' => $this->rbacUserRoleRepo
];
    }
    ...
    protected function withRowLock(callable $locker, int|string|array $id, callable $fetch, string $mode = 'wait'): mixed
    {
        ...
        return $this->txn(function () use ($locker, $id, $fetch, $mode) {
            ...
            if (!$row) {
                ...
                throw new \BlackCat\Database\Packages\RbacUserRoles\ModuleException("Record {$idStr} not found for locking.");
            }
